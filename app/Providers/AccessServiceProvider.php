...
use Modules\Core\Services\Access\Access;
...
class AccessServiceProvider extends ServiceProvider
{
    ...
    public function registerFacade()
    {
        $this->app->booting(function () {
            ...
            $loader->alias('Access', \Core\Services\Access\Facades\Access::class);
        });
    }
    ...
    protected function registerBladeExtensions()
    {
        ...
        Blade::directive('roles', function ($roles) {
            return "<?php if (Auth::user()->hasRoles({$roles})): ?>";
        });

        Blade::directive('needsroles', function ($roles) {
            return '<?php if (Auth::user()->hasRoles('.$roles.', true)): ?>';
        });
        ...
        Blade::directive('permission', function ($permission) {
            return "<?php if (Auth::user()->allow({$permission})): ?>";
        });
        ...
        Blade::directive('permissions', function ($permissions) {
            return "<?php if (Auth::user()->allowMultiple({$permissions})): ?>";
        });

        Blade::directive('needspermissions', function ($permissions) {
            return '<?php if (Auth::user()->allowMultiple('.$permissions.', true)): ?>';
        });
